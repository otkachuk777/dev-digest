...
4. **Cache:** `cache.json` = `{"<skill>|<sha of skill's SKILL.md>|<git hash-object file>": [findings]}`. Per skill, only files with no cache entry are reviewed now. **Always write it back after step 5** — a follow-up run after a fix then reviews only the files the fix touched. Reuse: when `architecture-reviewer` already ran on the same fingerprint, take its findings for `onion-architecture` and `frontend-ui-architecture` instead of re-reviewing those skills.
5. **Review:** write the shared inputs ONCE to `.claude/.pr-self-review/`: `context.md` (file list, `git diff --stat`, guard results from step 2) and one diff per group (`git diff $(git merge-base main HEAD) -- <group files> > diff-<group>.patch`; on a re-run after a fix, the delta only: `git diff <last reviewed commit> -- …`). Then launch one `Agent` (`model: sonnet`) **per group** from [skill-map.md](references/skill-map.md) "Review groups" that has uncached files, using the prompt in [agent-prompt.md](references/agent-prompt.md) (shared prefix first, per-group part last). **All in one message, in parallel.** Store results in the cache.
6. **Verify:** for each LLM-produced critical, first try to settle its premise deterministically in the main session — e.g. "existing rows / callers" → `git grep <writer>` across **all** branches (`git branch -a`), not just the merge-base; "column/file exists at that point" → read the migration order or `git cat-file -e <rev>:<path>`. Launch a verify agent (same file, [agent-prompt.md](references/agent-prompt.md)) only when that does not settle it. Refuted → downgrade to major.
